...
export function useMultibandVolume(
  ...
  const [frequencyBands, setFrequencyBands] = useState<number[]>(() => {
    if (!mediaStream) return new Array(opts.bands).fill(0)
    return new Array(opts.bands).fill(0)
  })
...
export const useBarAnimator = (
  ...
  const [currentFrame, setCurrentFrame] = useState<number[]>(() => sequence[0] || [])
...
const BarVisualizerComponent = forwardRef<HTMLDivElement, BarVisualizerProps>(
  (
    ...
    const [fakeVolumeBands, setFakeVolumeBands] = useState<number[]>(() => {
      if (!demo || (state !== "speaking" && state !== "listening")) {
